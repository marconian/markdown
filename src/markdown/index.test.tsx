...
describe('Markdown', () => {
    ...
    it('supports nested inline emphasis and inline code combinations', () => {
        const markdown = ['___combo___ __bold__ _italic_ **bold `code` text**', '', '**escaped characters, *and* repeated**'].join('\n\n');
        const { container } = render(<Markdown>{markdown}</Markdown>);

        const boldItalics = Array.from(container.querySelectorAll('b i'));
        expect(boldItalics.some((node) => node.textContent === 'combo')).toBe(true);
        ...
        const italicTexts = Array.from(container.querySelectorAll('i'))
            .map((node) => node.textContent?.trim())
            .filter(Boolean);
        expect(italicTexts).toContain('italic');

        const escapedBold = bolds.find((node) => node.textContent?.includes('escaped characters'));
        expect(escapedBold).not.toBeUndefined();
        const nestedItalic = escapedBold?.querySelector('i');
        expect(nestedItalic).not.toBeNull();
        expect(nestedItalic?.textContent).toBe('and');
    });

    it('keeps inline code spans intact across emphasis and multi-backtick blocks', () => {
        const markdown = ['Mixing **bold text with `inline code` inside** the sentence.', '', 'Inline ``code with `backticks` inside`` remains intact.'].join('\n\n');
        const { container } = render(<Markdown>{markdown}</Markdown>);
        ...
        const multiBacktickCode = screen.getByText('code with `backticks` inside', { selector: 'code' });
        expect(multiBacktickCode).toBeInTheDocument();
    ...
    it('renders mixed footnote scenarios within ordered lists without promoting continuations to code blocks', () => {
        const markdown = [
            ...
            '   - Another nested bullet mixes autolinks https://example.com and inline `code` before calling a different footnote[^html-note].',
